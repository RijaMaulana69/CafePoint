    ...
    <div class="flex items-center justify-between mb-6">
        <div class="w-full md:w-1/3">
            <input
                ...
                placeholder="Cari kode, nama, atau telepon customer..."
        ...
        <table class="w-full text-left border-collapse">
            <thead class="bg-gray-100 border-b">
                <tr>
                    ...
                    <th class="px-5 py-3 text-sm font-semibold text-gray-600">Nama Pelanggan</th>
            ...
            <tbody class="divide-y divide-gray-200">
                @forelse ($customers as $customer)
                    <tr class="hover:bg-gray-50">
                        <td class="px-5 py-4 text-sm">{{ ($customers->currentPage() - 1) * $customers->perPage() + $loop->iteration }}</td>
                        <td class="px-5 py-4 text-sm font-mono font-semibold text-blue-600">{{ $customer->code }}</td>
                        <td class="px-5 py-4 text-sm font-medium text-gray-900">{{ $customer->name }}</td>
                        <td class="px-5 py-4 text-sm text-gray-600">{{ $customer->phone ?? '-' }}</td>
                        <td class="px-5 py-4 text-sm text-gray-600">{{ $customer->email ?? '-' }}</td>
                        <td class="px-5 py-4 text-sm text-gray-600 max-w-xs truncate">{{ $customer->address ?? '-' }}</td>
                        ...
                        <td class="px-5 py-4 text-sm text-center space-x-2">
                            <button wire:click="edit({{ $customer->id }})" class="text-blue-600 hover:text-blue-800 font-medium">Edit</button>
                            <button
                ...
                @empty
                    <tr>
                        <td colspan="8" class="text-center py-8 text-gray-500">Belum ada data customer.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4 py-6 overflow-y-auto">
            <div class="w-full max-w-lg bg-white rounded-2xl shadow-xl my-8">
                ...
                <div class="p-6 space-y-4">
                    ...
                    {{-- Grid 2 Kolom: Kode & Nama Pelanggan --}}
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input
                                ...
                                class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500 uppercase font-mono font-semibold">
                            @error('code')
                    ...
                    <div class="flex items-center gap-3 pt-2">
                        ...
                        <label for="is_active_cust" class="text-sm font-medium text-gray-700">
                            Member / Customer Aktif
                        </label>
                    </div>
                ...
                <div class="flex justify-end gap-3 px-6 py-4 border-t bg-gray-50 rounded-b-2xl">
                    ...
                    <button
                        ...
                        <span wire:loading.remove>
                            {{ $customerId ? 'Update Customer' : 'Simpan Customer' }}
                        </span>
                        <span wire:loading>
                            Menyimpan...
                        </span>
